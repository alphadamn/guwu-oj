/* Extracted from templates/admin/base_site.html inline <script> (2026-09-13).
 * ============================================================
 * Guwu: SimpleUI 下日历/时钟弹窗的定位重写
 * - 不阻止 Django 原有的 click / submit 事件；不调用 e.preventDefault/stopPropagation
 * - 只做三件事：覆盖 openCalendar/openClock/dismissCalendar/dismissClock，
 *   用 getBoundingClientRect 把弹窗按视口坐标放置，并挂到顶层 #__guwu_popup_root
 * ============================================================ */
(function(){
    'use strict';
    ...
    function getPopupRoot(){
        ...
        if(!root){
            ...
            s.overflow = 'visible'; s.zIndex = '2147483000';
            s.pointerEvents = 'none'; s.transform = 'none';
            s.webkitTransform = 'none'; s.willChange = 'auto'; s.contain = 'none';
    ...
    function findLinkForBox(box){
        ...
        if(!m) return null;
        var kind = m[1].toLowerCase(), idx = m[2];
        if(kind === 'calendar') return document.getElementById('calendarlink' + idx);
    ...
    function placePopup(box, link, xOffset, defaultTopOffset){
        ...
        box.style.transform = 'none'; box.style.webkitTransform = 'none';
        box.style.willChange = 'auto'; box.style.margin = '0';
        box.style.float = 'none';
        box.style.display = 'block';
        box.style.visibility = 'visible';
        box.style.pointerEvents = 'auto'; box.style.zIndex = '2147483000';
    ...
    function patch(){
        if(window.DateTimeShortcuts && !window.DateTimeShortcuts.__guwuPatched){
            ...
            // 日历
            var origOpenCal = window.DateTimeShortcuts.openCalendar;
            window.DateTimeShortcuts.openCalendar = function(num){
                // 1) 先让 Django 原函数执行（它会设置 display + 绑定 dismiss 监听）
                try{ origOpenCal.call(this, num); }catch(e){}
                // 2) 再用我们的坐标覆盖
                try{
            ...
            // 时钟
            var origOpenClock = window.DateTimeShortcuts.openClock;
            window.DateTimeShortcuts.openClock = function(num){
                try{ origOpenClock.call(this, num); }catch(e){}
                try{
                    ...
                    if(clockBox){
                        ...
                        clockBox.style.visibility = 'visible';
                        placePopup(clockBox, clockLink, 17, 30);
                    }
                }catch(e){}
            };

            // dismiss：保持与 open 对称；display:none + visibility:hidden
            window.DateTimeShortcuts.dismissCalendar = function(num){
                try{
                    ...
                    if(box){ box.style.display = 'none'; box.style.visibility = 'hidden'; }
                ...
                var mo = new MutationObserver(function(mutations){
                    for(var i = 0; i < mutations.length; i++){
                        ...
                        for(var j = 0; j < added.length; j++){
                            ...
                            if(n.nodeType === 1){
                                var hit = false;
                                if(n.id && /^(calendarbox\d+|clockbox\d+)$/i.test(n.id)) hit = true;
                                if(!hit && n.querySelector && n.querySelector('[id^="calendarbox"], [id^="clockbox"]')) hit = true;
                                if(hit){
        ...
        setTimeout(applyToAllNewBoxes, 2000);
    }
